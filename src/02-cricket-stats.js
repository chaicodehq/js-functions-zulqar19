...
export const getPlayerCard = (player) => {
  ...
  return { name: player.name, strikeRate: calcStrikeRate(player.runs , player.balls), economy: calcEconomy(player.runsConceded , player.overs), battingAvg: calcBattingAvg(player.totalRuns , player.innings , player.notOuts), isAllRounder : isAllRounder(player.battingAvg , player.economy) }
